...
async function UpdateDoiTen(type)
{
  ...
  if (guildata)
  {
    let bancheckchannel    = client.channels.cache.get(guildata.bancheckid)
...
async function UpdateBlacklistNganh(type)
{
  ...
  if (updatetime.length != 0  && updatetime[0].date != datenow)
  {
    ...
    if (bandata.length == 0 ) 
      banchannel.send(" `Hôm Nay Không Có " + type + "`")
    else  {
      let rolelist = ""
...
async function GetImageNews( photo , title , content)
{
  ...
  ctx.globalAlpha = 0.6;
  logo = await loadImage('./picture/123.png')
  ctx.drawImage(logo, 540, 10, 150, 50)

  ctx.globalAlpha = 0.1;
  ironman= await loadImage('./picture/logoo.png')
  ctx.drawImage(ironman, 100,  400, 500, 500)
...
function restart()
{
  ...
  let heroku = new Heroku({ token: 'ef5def13-80c7-4a06-b12c-e1cc4bd62eaf' });
  heroku.delete('/apps/rubot/dynos/web').then( x => console.log(x) ).catch(() => restart())
} 
